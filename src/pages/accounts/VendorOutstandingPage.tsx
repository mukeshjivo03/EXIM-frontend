...
import { getVendorOutstanding, type VendorOutstandingEntry } from "@/api/sapSync";
...
let cachedEntries: VendorOutstandingEntry[] = [];
let hasSynced = false;
...
export default function VendorOutstandingPage() {
  ...
  async function handleSync() {
    ...
    try {
      ...
      setSynced(true);
      setSyncedAt(now);
      toast.success(`Loaded ${sorted.length} vendor outstanding entries`);
    } catch (err) {
      toast.error(getErrorMessage(err, "Failed to fetch vendor outstanding"));
    } finally {
      setSyncing(false);
    }
  }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
        <div>
          ...
          <p className="text-xs sm:text-sm text-muted-foreground mt-1">
            Vendor outstanding balances from SAP
            {syncedAt && (
          ...
          >
            ...
            {syncing ? (
              ...
              <>
                ...
                <span className="hidden sm:inline">Sync Vendor Outstanding</span>
              </>
            )}
          </Button>
        </div>
        ...
        <Card className="card-hover">
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              {entries.filter((e) => e.Balance > 0).length} vendors
            </p>
          </CardContent>
        </Card>
      ...
      <Card className="card-hover shimmer-hover">
        <CardHeader className="px-4 sm:px-6">
          <div className="flex items-center justify-between gap-3 flex-wrap">
            <div>
              <CardTitle className="text-base sm:text-lg">Vendor Outstanding Balances</CardTitle>
              <CardDescription className="text-[10px] sm:text-xs">
                {synced
                  ? hasFilters
                    ? `${filteredEntries.length} of ${entries.length} vendors`
                    : `${entries.length} vendors`
                  : 'Click "Sync Vendor Outstanding" to load outstanding balances from SAP'}
              </CardDescription>
            </div>
            {synced && entries.length > 0 && (
                      ...
                      <div className="flex flex-col items-center gap-2 text-muted-foreground py-12">
                        ...
                        <p className="text-sm">
                          {hasFilters
                            ...
                            : "Sync the vendor outstanding from SAP to see entries."}
                        </p>
                        {hasFilters && (
                          <Button
                            size="sm"
                      ...
                      <TableRow>
                        <TableCell colSpan={6}>
                          <div className="flex flex-col items-center gap-2 text-muted-foreground py-12">
                            ...
                            <p className="text-sm">Sync the vendor outstanding first.</p>
                          </div>
                        </TableCell>
                      </TableRow>
                    ) : (
